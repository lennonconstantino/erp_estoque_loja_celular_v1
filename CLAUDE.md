...
> Estado atual: **todos os bounded contexts estão implementados em Go** (`iam`, `clientes`, `fornecedores`, `catalogo`, `estoque`, `compras`, `vendas`, `relatorios`), seguindo este molde. O frontend cobre todas as telas e compartilha o kit de UI em `@/components/ui`. A Fase 9 (deploy) está **no ar** desde 2026-07-01: backend e frontend rodando no Railway (projeto `erp-estoque`) contra Postgres no Supabase, com o ciclo de negócio validado em produção. Dois detalhes de infra descobertos na subida: `DATABASE_URL` usa o **Session Pooler** do Supabase (IPv4 — a conexão direta é IPv6-only e o egress do Railway é IPv4) e o `frontend/Dockerfile` faz o nginx escutar em `$PORT`. Ver [docs/licoes-aprendidas.md](docs/licoes-aprendidas.md), [docs/todos.md](docs/todos.md) e [docs/setup/railway-deployment.md](docs/setup/railway-deployment.md).
...
Todas as telas compõem o **kit de UI** em `@/components/ui` (casca `PageShell` com `Sidebar` fixa + paleta de comandos ⌘K, `DataTable`, `Tabs`, `Modal`, `Field`, `StatusBadge`, `Button`) e estilizam **só com tokens semânticos** do tema — há **Dark/Light mode** via `@/lib/theme` (classe `.dark` no `<html>`, tokens em `src/index.css`), então nada de cores cruas. A linguagem visual completa (kit, tokens/tema, paleta + contraste, a11y) está em [docs/reference/design-system.md](docs/reference/design-system.md); convenções operacionais em [docs/setup/frontend-setup.md](docs/setup/frontend-setup.md) e [frontend/CLAUDE.md](frontend/CLAUDE.md).
