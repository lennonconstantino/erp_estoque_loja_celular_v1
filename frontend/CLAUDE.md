...
- **Kit de UI compartilhado** em `@/components/ui` é a fonte única do padrão visual: `PageShell` (casca: `Sidebar` fixa + cabeçalho com `CommandPalette`/`ThemeToggle`), `DataTable<T>` (tabela com ordenação por `sortAccessor`), `Button`/`buttonClasses` (variantes `primary`/`secondary`/`danger`/`success`/`ghost`; sem azul/índigo ad-hoc), `Tabs<T>` (abas pill), `StatusBadge`, `Modal`, `Field`/`inputClasses` (+ `inputClassesCompact`/`compactLabelClass` para grades densas de itens). Páginas novas **compõem** esses primitivos em vez de remontar cabeçalho/tabela/modal na mão — foi a ausência dessa camada que fez as telas divergirem. Detalhes e tabela completa em [docs/reference/design-system.md](../docs/reference/design-system.md).
- **Estilize só com tokens semânticos do tema** (`bg-card`, `text-foreground`, `border-border`, `text-muted-foreground`, `text-primary`, `text-destructive`…), nunca cores cruas (`text-red-600`, `bg-white`). O tema **Dark/Light** vem de `@/lib/theme` (`ThemeProvider` em `App.tsx` alterna a classe `.dark` no `<html>`; tokens definidos em `src/index.css`). Cor crua não acompanha a troca de tema.
- **Notificações** via `sonner`: `<Toaster>` é montado uma vez em `App.tsx`; use `toast.success/error` nas páginas (não crie banners ad-hoc para feedback transitório).
- **Acessibilidade (axe passa limpo, manter assim):** compor o kit já entrega a baseline. Ao escrever telas novas: `<select>`/`<input>` fora de `Field` precisa de `aria-label`; mensagem de erro com `role="alert"`; botão só-ícone com `aria-label`; seções internas começam em `<h2>` (o `<h1>` é do `PageShell`); não escurecer `--muted-foreground` nem usar `opacity-*` em texto muted; verde como texto = `green-700 dark:green-400` (ou `green-800` sobre tinta verde). Detalhes em [docs/reference/design-system.md → Acessibilidade](../docs/reference/design-system.md#acessibilidade-a11y).
