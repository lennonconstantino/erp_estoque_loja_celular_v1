...
// políticas de resiliência (Retry → CircuitBreaker → Bulkhead) aplicadas aqui,
// fora do domínio.
package cep
...
// resposta espelha o JSON do ViaCEP.
...
func (g *Gateway) Lookup(ctx context.Context, cep string) (domain.Endereco, error) {
	cep = domain.NormalizarCPF(cep) // mantém só dígitos
	if len(cep) != 8 {
...
func (g *Gateway) fetch(ctx context.Context, cep string) (domain.Endereco, error) {
	...
	if err != nil {
		return domain.Endereco{}, err // transitório: elegível a retry
	}
	...
	case resp.StatusCode >= 500:
		return domain.Endereco{}, fmt.Errorf("viacep: status %d", resp.StatusCode) // retriável
	default:
